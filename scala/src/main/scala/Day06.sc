...
// Input parsing.
case class Area(xi: Int, yi: Int, xe: Int, ye: Int )

object Area {
  def apply(s: String) : Area =
    s.trim.split(' ').flatMap( _.split(',') ) match {
      case Array(xi, yi, _, xe, ye) => Area( xi.toInt, yi.toInt, xe.toInt, ye.toInt )
    }
}
...
case class Toggle(a: Area) extends Action
case class TurnOn(a: Area) extends Action
case class TurnOff(a: Area) extends Action

var actions = input.split('\n')
              .map {
                      case m if m.startsWith("toggle") => Toggle( Area(m.substring(6) ) )
                      case m if m.startsWith("turn on") => TurnOn( Area(m.substring(7) ) )
                      case m if m.startsWith("turn off") => TurnOff( Area(m.substring(8) ) )
              }
...
def isInArea( point: (Int,Int,Int), a: Area) : Boolean = {
  val Area(xi, yi, xe,ye) = a
  val (x,y,_) = point
  x >= xi && y >= yi && x <= xe && y <= ye
}
def execute( list: Seq[(Int,Int,Int)], action: Action ) : Seq[(Int,Int,Int)] = {
  list.map {
             case p if isInArea(p,action.a) => action match {
               case Toggle(_) => (p._1, p._2, Math.abs( p._3 - 1) )
               case TurnOff(_) => (p._1, p._2, 0 )
               case TurnOn(_) => (p._1, p._2, 1 )
             }
             case p => p
           }
}

val lightOnCount = actions
                  .foldLeft[Seq[(Int,Int,Int)]](lights)( execute )
                  .map(_._3)
                  .sum                           //543903

// Second puzzle.
def execute2( list: Seq[(Int,Int,Int)], action: Action ) : Seq[(Int,Int,Int)] = {
  list.map {
             case p if isInArea(p,action.a) => action match {
               case Toggle(a) => (p._1, p._2, p._3 + 2 )
               case TurnOff(a) => (p._1, p._2, Math.max( 0, p._3 - 1) )
               case TurnOn(a) => (p._1, p._2, p._3 + 1 )
             }
             case p => p
           }
}

val totalBrightness = actions
                       .foldLeft[Seq[(Int,Int,Int)]](lights)( execute2 )
                       .map(_._3)
                       .sum                      // 14687245
